	 ...
	 public function drawCustomMenuItem($category= null, $level = 0, $last = false,$item= null )
    {
		...
		if($class_new_sale) {
			if(!in_array($id,$arr_catsid)) {
				$html[] = '<a href="#" class="pt_cate '.$is_active. $class_new_sale.'">';
			} else {
				$html[] = '<a href="'.$link.'" class="pt_cate '.$is_active. $class_new_sale.'">';
			}
		}	
		else {
			if(!in_array($id,$arr_catsid)) {
        ...
        foreach ($children as $child)
        {
           // echo "<pre>"; print_r($child->getData()); echo "</pre>";
            if ($child->getIsActive())
            {
				...
				if($class_new_sale)
					$class_new_sale = ' class="'.$class_new_sale.'"';
                ...
                // }else{
                    $html.= '<a class="itemMenuName level' . $level . $active . $ClassNoChildren . '" href="'.$sub_link.'"><span'.$class_new_sale.'>' . $name . '</span>' . $is_sale.$is_new . '</a>';
                // }
